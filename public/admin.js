...
function render(){document.querySelector('#pageTitle').textContent=titles[state.view];document.querySelector('#breadcrumb').textContent=titles[state.view];const views={dashboard,merchants,leads,products,promos,reviews,orders,payments,phones,recharges,finance,settlements:settlementsView,payouts:payoutsView,patrol:patrolView,scores:scoresView,broadband,plates,afterSales,notifications,logs,settings};document.querySelector('#content').innerHTML=views[state.view]();bindView()}
