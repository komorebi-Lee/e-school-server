...
const titles={dashboard:'经营概览',merchants:'商家入驻',products:'商品中心',promos:'话费活动',reviews:'商品评价',orders:'电瓶车订单',payments:'支付单',phones:'电话卡订单',recharges:'话费权益',finance:'财务流水',broadband:'宽带资格',plates:'牌照辅助',afterSales:'售后工单',notifications:'站内通知',logs:'操作日志',settings:'运营设置',settlements:'商家结算',payouts:'商家提现',patrol:'超时预警'};
const statuses={PENDING_PAYMENT:'待支付',PAID:'已支付',FULFILLING:'配送中',COMPLETED:'已完成',CANCELLED:'已取消',PENDING:'待支付',REFUNDED:'已退款',PENDING_REALNAME:'待实名',ACTIVATED:'已激活',PENDING_CREDIT:'待到账',CREDITED:'已到账',PENDING_VERIFY:'待核验',APPROVED:'已通过',REJECTED:'未通过',MATERIAL_PENDING:'待材料',REVIEWING:'处理中',SUBMITTED:'待处理',CLOSED:'已关闭',AFTER_SALE:'售后中',PUBLISHED:'已展示',HIDDEN:'已隐藏',PENDING_SETTLE:'可结算',SETTLED:'已结算',PENDING_DELIVERY:'待交付核验',IN_ACCOUNT_PERIOD:'账期中',FROZEN:'售后冻结',PAYOUT_REQUESTED:'提现待审核',PENDING_REVIEW:'待审核',OPEN:'待认领',ACKNOWLEDGED:'已认领',RESOLVED:'已关闭',OVERDUE:'已超时',WARNING:'即将超时'};
...
const collections={orders:'orders',payments:'paymentOrders',phones:'phoneCardOrders',recharges:'rechargeOrders',broadband:'broadbandApplications',plates:'plateApplications',afterSales:'afterSales',finance:'financeEvents',settlements:'settlements',payouts:'payoutRequests',patrol:'slaAlerts'};
...
function notifications(){const items=(state.data.notifications||[]).filter(match);const rows=items.slice(0,80).map(x=>`<tr><td><strong>${esc(x.title)}</strong><small>${esc(x.type)}</small></td><td>${esc(x.content)}</td><td><span class="badge ${x.read?'green':'orange'}">${x.read?'已读':'未读'}</span></td><td>${fmtDate(x.createdAt)}</td></tr>`);return toolbar(items.length)+table(['通知','内容','状态','时间'],rows,items.length)}
...
function render(){document.querySelector('#pageTitle').textContent=titles[state.view];document.querySelector('#breadcrumb').textContent=titles[state.view];const views={dashboard,merchants,leads,products,promos,reviews,orders,payments,phones,recharges,finance,settlements:settlementsView,payouts:payoutsView,patrol:patrolView,broadband,plates,afterSales,notifications,logs,settings};document.querySelector('#content').innerHTML=views[state.view]();bindView()}
