...
function reviews(){const reviews=state.data.productReviews||[];const items=reviews.filter(match).filter(statusMatch).map(review=>({...review,productName:(state.data.products||[]).find(product=>product.id===review.productId)?.name||review.productId,visibility:review.visibility||'PUBLISHED'}));const gallery=(images)=>{const evidence=Array.isArray(images)?images:[];return evidence.length?`<div class="review-gallery">${evidence.slice(0,3).map(image=>`<a href="${esc(image)}" target="_blank" rel="noopener"><img src="${esc(image)}" alt="评价图片证据"></a>`).join('')}</div>`:'—'};const rows=items.map(review=>`<tr><td><strong>${esc(review.customerName||'匿名同学')}</strong><small>已购核验</small></td><td><strong>${esc(review.productName)}</strong><small>${esc(review.id)}</small></td><td>${'★'.repeat(review.rating)}</td><td>${esc(review.content)}${gallery(review.images)}</td><td>${esc(review.reply?.content||'未回复')}</td><td>${fmtDate(review.createdAt)}</td><td><span class="badge ${review.visibility==='HIDDEN'?'red':'green'}">${label(review.visibility)}</span></td><td><button class="table-button toggle-review" data-id="${review.id}" data-visibility="${review.visibility==='HIDDEN'?'PUBLISHED':'HIDDEN'}">${review.visibility==='HIDDEN'?'恢复展示':'隐藏'}</button></td></tr>`);return toolbar(items.length,{statusesList:['PUBLISHED','HIDDEN']})+table(['用户','商品','评分','内容 / 图片证据','商家回复','时间','状态','操作'],rows,items.length)}
