...
function dashboard(){const m=state.data.metrics;const d=state.data;const queues=state.data.operationsInsights?.orderQueues||{};const favoriteDemand=(d.favoriteDemandProducts||[]).sort((a,b)=>b.favoriteCount-a.favoriteCount).slice(0,5);return`<div class="welcome"><div><h2>上午好，运营管理员</h2><p>这里汇总了今天最需要关注的交易与履约事项。</p></div><div class="date-chip">${new Date().toLocaleDateString('zh-CN',{month:'long',day:'numeric',weekday:'long'})}</div></div><div class="metric-grid">${metric('累计成交额',money(m.revenueInCents),'仅统计已支付业务',true)}${metric('已支付订单',m.paidOrders,'电瓶车、电话卡与牌照')}${metric('待处理事项',m.pending,'不含待支付和已取消')}${metric('可售偏低商品',m.lowStock,'可售库存已达到补货阈值')}${metric('收藏需求',m.favoriteCount||0,'用于选品与补货参考')}${metric('售后超时',m.afterSaleOverdue||0,'超过承诺响应时限')}${metric('履约超时预警',(state.data.slaSummary?.overdueCount||0),`即将超时 ${state.data.slaSummary?.warningCount||0} 条`)}${metric('商家服务分均值',(state.data.merchantScoreSummary?.averageScore||0),`限流 ${state.data.merchantScoreSummary?.limitedCount||0} · 暂停上新 ${state.data.merchantScoreSummary?.restrictedCount||0}`)}</div><div class="dashboard-grid"><section class="panel"><div class="panel-head"><h2>运营待办</h2><span>点击进入业务列表</span></div><div class="task-list">${task('商家入驻审核','审核新入驻商家资质',(d.merchants||[]).filter(x=>x.status==='REVIEWING').length,'merchants')}${task('电瓶车履约跟进','处理已支付和售后订单',(d.orders||[]).filter(x=>['PAID','FULFILLING','AFTER_SALE'].includes(x.status)).length,'orders')}${task('电话卡实名激活','核对运营商实名结果',(d.phoneCardOrders||[]).filter(x=>x.status==='PENDING_REALNAME').length,'phones')}${task('校园牌照辅助','跟进材料与审核进度',(d.plateApplications||[]).filter(x=>['MATERIAL_PENDING','REVIEWING'].includes(x.status)).length,'plates')}${task('双人宽带资格','核验两位同学购卡状态',(d.broadbandApplications||[]).filter(x=>x.status==='PENDING_VERIFY').length,'broadband')}${task('话费权益到账','确认充值优惠到账',(d.rechargeOrders||[]).filter(x=>x.status==='PENDING_CREDIT').length,'recharges')}${task('超时预警认领','巡检发现的逾期与临期事项',(state.data.slaSummary?.openCount||0),'patrol')}${task('商家提现审核','核对收款账户后确认打款',(d.payoutRequests||[]).filter(x=>x.status==='PENDING_REVIEW').length,'payouts')}${task('服务分整改跟进','限流或暂停上新的商家',((state.data.merchantScoreSummary?.limitedCount||0)+(state.data.merchantScoreSummary?.restrictedCount||0)),'scores')}${task('商品复核','限流商家新增商品待复核',(state.data.pendingPublishProducts||[]).length,'scores')}</div></section><section class="panel"><div class="panel-head"><h2>库存预警</h2><span>阈值 ${lowStockThreshold} 件</span></div><div class="stock-list">${(d.products||[]).filter(p=>p.active!==false&&Number(p.availableStock??p.stock??0)<=lowStockThreshold).sort((a,b)=>Number(a.availableStock??a.stock??0)-Number(b.availableStock??b.stock??0)).slice(0,8).map(p=>{const available=Number(p.availableStock??p.stock??0);return`<div class="stock-item"><span>${esc(p.name)}<small>${esc(p.favoriteDemandText||'')}</small></span><strong class="${available<=lowStockThreshold?'low':''}">${available}件</strong></div>`}).join('')||'<p class="muted-empty">暂无达到阈值的商品</p>'}</div></section><section class="panel"><div class="panel-head"><h2>收藏需求</h2><span>学生主动关注</span></div><div class="stock-list">${favoriteDemand.map(p=>`<div class="stock-item"><span>${esc(p.name)}<small>${esc(p.favoriteDemandText)} · 库存 ${p.availableStock}</small></span><strong>${p.favoriteCount}</strong></div>`).join('')||'<p class="muted-empty">暂无收藏需求</p>'}</div></section><section class="panel"><div class="panel-head"><h2>履约联动</h2><span><button class="text-button" data-goto="orders">进入订单</button></span></div><div class="stock-list">${(queues.pendingAccept || []).map(order => `<div class="stock-item"><span>待接单 · ${esc(order.orderNo)}<small>${esc(order.merchantName)} · ${esc(order.customerName)} ${esc(order.contactPhone)}</small></span><strong>${money(order.amountInCents)}</strong></div>`).join("")}${(queues.delivering || []).map(order => `<div class="stock-item"><span>配送中 · ${esc(order.orderNo)}<small>${esc(order.merchantName)} · ${esc(order.address || order.timeSlot || "校内履约中")}</small></span><strong>${esc(order.timeSlot || "履约中")}</strong></div>`).join("")}${(queues.unrepliedMessages || []).map(order => `<div class="stock-item"><span>待回复 · ${esc(order.orderNo)}<small>${esc(order.merchantName)} · ${esc(order.text || "用户留言")}</small></span><strong>${esc(fmtDate(order.dueAt))}</strong></div>`).join("")}${!(queues.pendingAccept || []).length && !(queues.delivering || []).length && !(queues.unrepliedMessages || []).length ? `<p class="muted-empty">当前没有待接单、配送中或待回复订单</p>` : ""}</div></section></div><section class="panel" style="margin-top:16px"><div class="panel-head"><h2>最近操作</h2><span>系统留痕</span></div><div class="log-list">${(d.auditLogs||[]).slice(0,5).map(log=>`<div class="log-item"><div><strong>${esc(log.action)}</strong><p>${esc(log.operator)} · ${esc(log.target)}</p></div><small>${fmtDate(log.createdAt)}</small></div>`).join('')}</div></section>`}
...
function products(){const items=state.data.products.filter(match);const rows=items.map(p=>{const available=Number(p.availableStock ?? p.stock ?? 0);const reserved=Number(p.reservedStock||0);const campaignBadge=p.campaignStatus==='ACTIVE'?'green':p.campaignStatus==='SCHEDULED'?'blue':'orange';return `<tr><td><strong>${esc(p.name)}</strong><small>${esc(p.description)}</small></td><td><span class="category">${esc(productCategoryLabels[p.category]||p.category)}</span></td><td>${money(p.effectivePriceInCents ?? p.priceInCents)}${p.campaignDiscountInCents?`<small>让利 ${money(p.campaignDiscountInCents)}</small>`:''}</td><td class="${available<=lowStockThreshold?'low':''}">${available}<small>总库存 ${p.stock}${reserved?` · 待支付占用 ${reserved}`:''}</small></td><td><span class="badge ${p.active?'green':'red'}">${p.active?'已上架':'已下架'}</span>${p.campaignStatus?`<small><span class="badge ${campaignBadge}">${esc(p.campaignStatusLabel)}</span> · ${p.campaignPaidOrderCount||0} 单 · ${money(p.campaignAmountInCents||0)}</small>`:''}</td><td><div class="row-actions"><button class="text-button detail-button" data-view="products" data-id="${p.id}">详情</button><button class="table-button edit-product" data-id="${p.id}">编辑</button></div></td></tr>`});return toolbar(items.length,{add:true})+table(['商品','分类','生效售价','可售/总库存','状态/促销','操作'],rows,items.length)}
...
function promos(){const items=state.data.rechargePromos||[];const rows=items.map(p=>{const period=[p.startsAt,p.endsAt].filter(Boolean).map(x=>fmtDate(x)).join(' → ')||'不限投放期';return `<tr><td><strong>充${p.pay}送${p.receive-p.pay}</strong><small>${esc(p.id)}</small></td><td>${money(p.pay*100)}</td><td>${money(p.receive*100)}</td><td><span class="badge ${p.status==='ACTIVE'?'green':p.status==='SCHEDULED'?'blue':'red'}">${esc(p.statusLabel||(p.active?'已上架':'已下架'))}</span><small>${esc(period)}</small></td><td>${p.linkedOrderCount||0}<small>有效 ${p.linkedPaidOrderCount||0} 单 · ${money(p.linkedAmountInCents||0)}</small></td><td><div class="row-actions"><button class="table-button edit-promo" data-id="${p.id}">编辑</button><button class="table-button toggle-promo" data-id="${p.id}" data-active="${!p.active}">${p.active?'下架':'上架'}</button></div></td></tr>`});return toolbar(items.length,{add:true})+table(['活动','充值','到账','投放状态','关联订单','操作'],rows,items.length)}
function reviews(){const reviews=state.data.productReviews||[];const items=reviews.filter(match).filter(statusMatch).map(review=>{const now=Date.now();const dueMs=review.replyDueAt?new Date(review.replyDueAt).getTime():0;const overdue=!!dueMs&&!review.reply?.content&&dueMs<now;const dueLabel=!dueMs||review.reply?.content?'':`${overdue?'回复已超时':'回复截止'} ${fmtDate(review.replyDueAt)}`;return{...review,productName:(state.data.products||[]).find(product=>product.id===review.productId)?.name||review.productId,visibility:review.visibility||'PUBLISHED',dueLabel,overdue}});const gallery=(images)=>{const evidence=Array.isArray(images)?images:[];return evidence.length?`<div class="review-gallery">${evidence.slice(0,3).map(image=>`<a href="${esc(image)}" target="_blank" rel="noopener"><img src="${esc(image)}" alt="评价图片证据"></a>`).join('')}</div>`:'—'};const ops=state.data.negativeReviewOperations||{};const opsPanel=`<section class="panel"><div class="panel-head"><h2>差评处理</h2><span>回复率 ${ops.replyRate||0}%</span></div><div class="metric-grid">${metric('差评总数',ops.total||0,'仅统计已展示差评')}${metric('待回复',ops.openCount||0,`超时 ${ops.overdueCount||0} · 临期 ${ops.dueSoonCount||0}`)}${metric('已回复',ops.repliedCount||0,'回复后计入服务分')}</div></section>`;const rows=items.map(review=>`<tr><td><strong>${esc(review.customerName||'匿名同学')}</strong><small>已购核验</small></td><td><strong>${esc(review.productName)}</strong><small>${esc(review.id)}</small></td><td>${'★'.repeat(review.rating)}</td><td>${esc(review.content)}${gallery(review.images)}</td><td>${esc(review.reply?.content||'未回复')}${review.dueLabel?`<small class="${review.overdue?'text-red':'muted'}">${esc(review.dueLabel)}</small>`:''}</td><td>${fmtDate(review.createdAt)}</td><td><span class="badge ${review.visibility==='HIDDEN'?'red':'green'}">${label(review.visibility)}</span></td><td>${Number(review.rating)<=2&&!review.reply?.content?`<button class="table-button urge-review-reply" data-id="${review.id}">催办回复</button>`:''}${review.lastUrge?`<small>已催办 ${fmtDate(review.lastUrge.urgedAt)}</small>`:''}<button class="table-button toggle-review" data-id="${review.id}" data-visibility="${review.visibility==='HIDDEN'?'PUBLISHED':'HIDDEN'}">${review.visibility==='HIDDEN'?'恢复展示':'隐藏'}</button></td></tr>`);return opsPanel+toolbar(items.length,{statusesList:['PUBLISHED','HIDDEN']})+table(['用户','商品','评分','内容 / 图片证据','商家回复','时间','状态','操作'],rows,items.length)}
function orders(){const items=state.data.orders.filter(match).filter(statusMatch);const rows=items.map(o=>{const collab=o.collaboration||{};const intervention=collab.intervention?.status==='REQUESTED';const pendingMessage=!!collab.unrepliedMessage;const merchant=(state.data.merchants||[]).find(x=>x.id===collab.merchantId)?.name||'平台自营';const deliveryLabel={PENDING_PAYMENT:'待支付',PAID:'待配送',FULFILLING:'配送中',COMPLETED:'已送达',CANCELLED:'已取消',AFTER_SALE:'售后处理中'}[o.status]||'—';const paymentStatus={UNPAID:'未支付',PAID:'支付成功',CANCELLED:'支付已取消',EXPIRED:'支付超时',PARTIALLY_REFUNDED:'部分退款',REFUNDED:'已退款'}[o.paymentStatus]||'未支付';const refundedQuantity=Number(o.refundedQuantity||0);const refundedText=refundedQuantity?` 已退 ${refundedQuantity} 件${o.partialRefundedInCents?` · ${money(o.partialRefundedInCents)}`:''}`:'';const deliveryCode=o.deliveryCode||'—';return `<tr><td><strong>${esc(o.orderNo)}</strong><small>${fmtDate(o.createdAt)}</small></td><td>${o.items.map(x=>esc(x.name)+(Number(x.quantity)>1?` × ${Number(x.quantity)}`:'')).join('、')}</td><td>${esc(merchant)}</td><td>${money(o.totalInCents)}<small>${esc(paymentStatus)}${esc(refundedText)}</small></td><td><span class="badge ${badgeClass(o.status)}">${label(o.status)}</span><small>${esc(deliveryLabel)}</small><small>交付码 ${esc(deliveryCode)}</small></td><td>${intervention?'<span class="badge red">待平台处理</span>':collab.unrepliedMessage?'<span class="badge orange">待回复咨询</span>':'<span class="badge green">正常流转</span>'}</td><td>${statusControl('orders',o)}</td><td><div class="row-actions"><button class="text-button detail-button" data-view="orders" data-id="${o.id}">查看详情</button><button class="text-button platform-collab" data-id="${o.id}" data-action="${intervention?'RESOLVE':'INTERVENE'}">${intervention?'平台处理':'介入'}</button></div></td></tr>`});return toolbar(items.length,{statusesList:options.orders})+table(['订单号','商品','商家','实付','支付/配送','协同','状态流转','操作'],rows,items.length)}
function serviceCollabRow(item,collection,typeLabel){const collab=item.collaboration||{};const intervention=collab.intervention?.status==='REQUESTED';const pendingMessage=!!collab.unrepliedMessage;return `<tr><td><strong>${esc(item.customerName||item.ownerPhone||item.phone||'用户')}</strong><small>${esc(item.id)}</small></td><td>${esc(typeLabel)}<small>${esc(item.planName||item.vehicleModel||'双人购卡宽带')}</small></td><td>${esc(item.phone||item.ownerPhone||'—')}</td><td><span class="badge ${badgeClass(item.status)}">${label(item.status)}</span></td><td>${intervention?'<span class="badge red">待平台处理</span>':pendingMessage?'<span class="badge orange">待回复咨询</span>':'<span class="badge green">正常流转</span>'}</td><td><div class="row-actions"><button class="text-button service-detail" data-id="${item.id}">对话</button><button class="text-button service-collab" data-collection="${collection}" data-id="${item.id}" data-action="${intervention?'RESOLVE':'INTERVENE'}">${intervention?'平台处理':'介入'}</button></div></td></tr>`}
...
const reconciliationTypeLabels={
  PROVIDER_PAYMENT_MISSING_LOCAL:'渠道有支付，本地缺失',
  LOCAL_PAYMENT_MISSING_PROVIDER:'本地有支付，渠道缺失',
  PAYMENT_AMOUNT_MISMATCH:'支付金额不一致',
  PROVIDER_REFUND_MISSING_LOCAL:'渠道有退款，本地缺失',
  LOCAL_REFUND_MISSING_PROVIDER:'本地有退款，渠道缺失',
...
function reconciliationPanel(){
  ...
  const paymentDifferenceCount=differences.filter(item=>item.type.includes('PAYMENT')).length;
  const refundDifferenceCount=differences.filter(item=>item.type.includes('REFUND')).length;
  const yesterday=localDateString(new Date(Date.now()-24*60*60*1000));
  const reportRows=reports.slice(0,8).map(report=>{
    const currentDifferences=report.differences||[];
...
function payments(){
  const items=(state.data.paymentOrders||[]).filter(match).filter(statusMatch);
  const merchantName=(id)=>(state.data.merchants||[]).find(x=>x.id===id)?.name||'平台自营';
  const rows=items.map(x=>{
    const linked=(state.data.settlements||[]).filter(s=>s.paymentId===x.id);
    const settleText=linked.length?linked.map(s=>`${merchantName(s.merchantId)}：${money(s.payableAmountInCents)} / ${label(s.settlementStatus)}`).join('；'):'平台自营';
    const firstPending=linked.find(s=>s.settlementStatus==='PENDING_SETTLE');
    const refund=x.refund||{};
    const refundPending=refund.status==='PENDING';
    const refundText=refundPending?'退款处理中':refund.status==='FAILED'?'退款失败':'';
    return `<tr><td><strong>${esc(x.paymentNo)}</strong><small>${esc(x.orderNo)}</small></td><td>${money(x.amountInCents)}</td><td>${esc(x.channel)}</td><td><span class="badge ${badgeClass(x.status)}">${label(x.status)}</span>${refundText?`<small>${esc(refundText)}</small>`:''}<small>${fmtDate(x.paidAt||x.createdAt)}</small></td><td>${esc(settleText)}</td><td><div class="row-actions"><button class="text-button detail-button" data-view="payments" data-id="${x.id}">查看详情</button>${firstPending?`<button class="text-button settle-merchant" data-merchant="${firstPending.merchantId}">确认结算</button>`:''}${refundPending?`<button class="text-button refund-refresh" data-id="${x.id}">查询退款结果</button>`:''}${x.status==='PAID'&&!refundPending?`<button class="text-button danger refund-payment" data-id="${x.id}">退款</button>`:''}</div></td></tr>`;
  });
  return reconciliationPanel()+toolbar(items.length,{statusesList:['PENDING','PAID','CANCELLED','PARTIALLY_REFUNDED','REFUNDED']})+table(['支付单','金额','渠道','状态','商家分账','操作'],rows,items.length);
}
...
function openServiceCollab(id){
  ...
  document.querySelector('#drawerBody').innerHTML=`
    ...
    <div class="detail-section"><h3>协同状态</h3><div class="detail-grid">
      ...
      ${detailItem('用户待回复',collab.unrepliedMessage?'有待回复留言':'已回复或暂无留言')}
...
function orderCollabDetail(order){
  const collab=order.collaboration||{};
  const fulfillment=order.fulfillment||{};
  const intervention=collab.intervention?.status==='REQUESTED';
  const pendingMessage=!!collab.unrepliedMessage;
  const roleLabels={USER:'用户',MERCHANT:'商家',PLATFORM:'平台'};
  const handoffs=[...(collab.handoffs||[])].reverse().slice(-10);
  return `<div class="detail-section"><h3>履约信息</h3><div class="detail-grid">
    ${detailItem('联系人',fulfillment.contactName||'—')}
    ${detailItem('联系电话',fulfillment.contactPhone||'—')}
    ${detailItem('配送日期',fulfillment.date||'—')}
    ${detailItem('配送时段',fulfillment.timeSlot||'—')}
    ${detailItem('配送地址',fulfillment.address||'—')}
    ${detailItem('交付码',order.deliveryCode||'—')}
  </div></div>
  <div class="detail-section"><h3>三方协同</h3><div class="detail-grid">
    ${detailItem('平台协助',intervention?'待平台处理':'无需介入')}
    ${detailItem('用户待回复',pendingMessage?'有待回复留言':'已回复或暂无留言')}
    ${detailItem('最新留言',collab.unrepliedMessage?.text||collab.messages?.[0]?.text||'—')}
  </div></div>
  <div class="detail-section"><h3>订单对话</h3>${serviceCollabMessages(collab)}</div>
  ${handoffs.length?`<div class="detail-section"><h3>履约轨迹</h3><div class="timeline">${handoffs.map(event=>`<div class="timeline-item"><strong>${esc(roleLabels[event.role]||'平台')} · ${esc(event.note||event.action||'状态更新')}</strong><span>${fmtDate(event.createdAt)}</span></div>`).join('')}</div></div>`:''}`;
}
...
const settingFieldLabels={brandName:'品牌名称',schoolName:'学校名称',campusName:'校区名称',servicePhone:'客服电话',serviceWechat:'客服微信',externalPlateFeeInCents:'自带车上牌服务费',deliveryFeeInCents:'校内配送费',commissionRatePercent:'平台佣金比例',deliveryResponseHours:'配送响应承诺',plateResponseHours:'上牌响应承诺',afterSaleResponseHours:'售后响应承诺',afterSaleResolutionHours:'售后处理时限',phoneCardActivationHours:'电话卡激活时限',rechargeCreditHours:'话费到账时限',broadbandVerifyHours:'宽带核验时限',payoutReviewHours:'提现审核时限',leadResponseHours:'线索跟进时限',reviewReplyHours:'差评回复时限',financeTaskResponseHours:'对账待办处理时限',patrolIntervalMinutes:'运营巡检间隔',lowStockThreshold:'低库存提醒阈值',serviceScoreLimitedThreshold:'服务分限流阈值',serviceScoreRestrictedThreshold:'服务分暂停上新阈值',productComplianceLowReviewThreshold:'低分下架阈值',productComplianceReviewSampleThreshold:'均分样本量阈值',productComplianceAverageRatingThreshold:'均分下架阈值',productComplianceOverdueAfterSaleThreshold:'售后超时下架阈值',paymentTimeoutMinutes:'待支付自动关闭',settlementPeriodDays:'商家结算账期',payoutMinimumInCents:'商家起提金额',deliveryTimeSlots:'配送时段',platformNotice:'平台提示语'};
...
function render(){document.querySelector('#pageTitle').textContent=titles[state.view];document.querySelector('#breadcrumb').textContent=titles[state.view];const views={dashboard,merchants,qualification:qualificationRenewalsView,leads,products,stock:stockMovementsView,promos,reviews,orders,payments,phones,recharges,finance,serviceCollabs,settlements:settlementsView,payouts:payoutsView,patrol:patrolView,scores:scoresView,broadband,plates,afterSales,notifications,logs,settings};document.querySelector('#content').innerHTML=views[state.view]();bindView()}
...
bindPayments=function(parent=document){
  parent.querySelector('#run-reconciliation')?.addEventListener('click',async()=>{
    const billDate=parent.querySelector('#reconciliation-date')?.value;
    if(!billDate)return showToast('请选择对账日期');
    try{
      showToast('正在拉取微信账单并核对');
      const result=await api('/api/admin/payment-reconciliations/run',{
        method:'POST',
        headers:{'content-type':'application/json'},
        body:JSON.stringify({billDate})
      });
      showToast(result?.status==='MATCHED'?'对账完成，账实相符':'对账完成，请处理差异');
      await load();
    }catch(error){
      showToast(error.message);
    }
  });
  parent.querySelectorAll('.refund-payment').forEach(b=>b.addEventListener('click',async()=>{
    const note=prompt('请填写退款备注','已与用户确认退款');
    if(note===null)return;
  ...
  parent.querySelectorAll('.refund-refresh').forEach(b=>b.addEventListener('click',async()=>{
    ...
    showToast(result?.data?.paymentOrder?.refund?.status==='REFUNDED'?'退款已确认完成':'退款仍在处理中');
    await load();
  }));
  parent.querySelectorAll('.settle-merchant').forEach(b=>b.addEventListener('click',async()=>{
    const merchant=(state.data.merchants||[]).find(x=>x.id===b.dataset.merchant);
    const account=merchant?.settlementAccount?`${merchant.settlementBank||''} ${merchant.settlementAccountName||merchant.name} ${merchant.settlementAccount}`:'请人工核对商家收款信息';
    const reference=prompt(`请确认已完成线下打款：\n${account}\n\n填写打款凭证/备注`,account==='请人工核对商家收款信息'?'':'平台线下打款');
    if(reference===null)return;
    await api(`/api/admin/merchants/${b.dataset.merchant}/settle`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({reference})});
    showToast('商家分账已确认结算');
    await load();
  }));
};
...
function openProduct(product){document.querySelector('#modalTitle').textContent=product?'编辑商品':'新增商品';document.querySelector('#productId').value=product?.id||'';document.querySelector('#productName').value=product?.name||'';document.querySelector('#productCategory').value=product?.category||'E_BIKE_NEW';document.querySelector('#productPrice').value=product?product.priceInCents/100:'';document.querySelector('#productStock').value=product?.stock??'';document.querySelector('#productSalePrice').value=product?.salePriceInCents?product.salePriceInCents/100:'';document.querySelector('#productSaleStartsAt').value=toDatetimeLocal(product?.saleStartsAt);document.querySelector('#productSaleEndsAt').value=toDatetimeLocal(product?.saleEndsAt);document.querySelector('#productDescription').value=product?.description||'';document.querySelector('#productImageUrl').value=product?.imageUrl||'';document.querySelector('#productActive').checked=product?.active!==false;toggleModal(true)}
...
function toDatetimeLocal(value){return value?new Date(new Date(value).getTime()-new Date(value).getTimezoneOffset()*60000).toISOString().slice(0,16):''}
function fromDatetimeLocal(value){return value?new Date(value).toISOString():''}
function openPromo(promo){document.querySelector('#promoModalTitle').textContent=promo?'编辑活动':'新增活动';document.querySelector('#promoId').value=promo?.id||'';document.querySelector('#promoPay').value=promo?.pay||'';document.querySelector('#promoReceive').value=promo?.receive||'';document.querySelector('#promoStartsAt').value=toDatetimeLocal(promo?.startsAt);document.querySelector('#promoEndsAt').value=toDatetimeLocal(promo?.endsAt);document.querySelector('#promoBadge').value=promo?.badge||'限时优惠';document.querySelector('#promoActive').checked=promo?promo.active!==false:true;togglePromoModal(true)}
...
function openLeadPanel(id){
  ...
  document.querySelector('#copyLeadInfo')?.addEventListener('click', async () => {
    const text = `${lead.name} ${lead.phone}\n${lead.businessType}：${lead.interest || ''}${lead.sourceNo ? `\n关联记录：${lead.sourceNo}${lead.sourceType ? `（${lead.sourceType}）` : ''}` : ''}`;
...
openDetail = function(view, id){
  ...
  ${detailItem('支付状态', item.paymentStatus === 'PAID' ? '支付成功' : '未支付')}
...
openDetail = function(view, id){
  ...
  const body=`<div class="detail-section"><h3>售后信息</h3><div class="detail-grid">
    ...
    ${item.type==='REFUND'&&Number(item.quantity||0)>0?detailItem('申请数量', `${item.quantity} 件`):''}
    ${item.type==='REFUND'&&Number(item.refundAmountInCents||0)>0?detailItem('预计退款', money(item.refundAmountInCents)):''}
...
bindView = function(){
  ...
  document.querySelectorAll('.urge-review-reply').forEach((button) => button.addEventListener('click', async () => {
    await api(`/api/admin/product-reviews/${button.dataset.id}/urge`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({})
    });
...
function scoresView() {
  ...
  const cards = `<div class="metric-grid">
    ...
    ${metric('差评待回复', negativeReviewOps.openCount || 0, `超时 ${negativeReviewOps.overdueCount || 0} · 临期 ${negativeReviewOps.dueSoonCount || 0} · 回复率 ${negativeReviewOps.replyRate || 0}%`)}
  </div>`;
  const templatePanel = `<section class="panel"><div class="panel-head"><h2>微信订阅消息模板</h2><span>服务分与订单提醒</span></div><div class="form-grid" style="grid-template-columns:repeat(2,1fr)">
    ...
    <label>差评回复提醒模板 ID<input id="negativeReviewReplyTemplate" value="${esc(state.data.settings?.negativeReviewReplyTemplateId || '')}"></label>
  ...
  document.querySelector('#saveScoreTemplates')?.addEventListener('click', async () => {
    await api('/api/admin/settings', {
      ...
      body: JSON.stringify({
        ...
        negativeReviewReplyTemplateId: document.querySelector('#negativeReviewReplyTemplate').value.trim(),
...
bindView = function () {
  ...
  document.querySelectorAll('.qualification-approve').forEach((button) => button.addEventListener('click', async () => {
    const note = prompt('填写复审备注（会同步给商家）', '新执照信息已核对');
    if (note === null) return;
    if (!note.trim()) return showToast('请填写复审备注');
    await api(`/api/admin/qualification-renewals/${button.dataset.id}/review`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ decision: 'APPROVE', reviewNote: note.trim() })
    });
    showToast('资质复审通过，正式资质已更新');
    await load();
  }));
  document.querySelectorAll('.qualification-reject').forEach((button) => button.addEventListener('click', async () => {
    const note = prompt('填写驳回原因（会同步给商家）', '执照图片不清晰或信息不完整');
    if (note === null) return;
    if (!note.trim()) return showToast('请填写驳回原因');
    await api(`/api/admin/qualification-renewals/${button.dataset.id}/review`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ decision: 'REJECT', reviewNote: note.trim() })
    });
    showToast('资质复审已驳回');
    await load();
  }));
