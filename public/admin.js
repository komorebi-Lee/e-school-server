...
// ===== 二手市集 / 学校论坛内容审核 =====
Object.assign(statuses, { ACTIVE: '在售', RESERVED: '已预留', SOLD: '已出', REMOVED: '已下架', PUBLISHED: '已发布' });

const baseBindViewWithCommunityModeration = bindView;
bindView = function () {
  baseBindViewWithCommunityModeration();
  document.querySelectorAll('.toggle-market').forEach((button) => button.addEventListener('click', () => {
    const status = button.dataset.status;
    api(`/api/admin/market-items/${encodeURIComponent(button.dataset.id)}/status`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ status })
    }).then(async () => {
      showToast(status === 'REMOVED' ? '商品已下架，卖家不可再修改' : '商品已恢复在售');
      await load();
    }).catch((error) => showToast(error.message));
  }));
  document.querySelectorAll('.toggle-forum').forEach((button) => button.addEventListener('click', () => {
    const status = button.dataset.status;
    api(`/api/admin/forum-posts/${encodeURIComponent(button.dataset.id)}/status`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ status })
    }).then(async () => {
