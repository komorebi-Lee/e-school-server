...
const titles={dashboard:'经营概览',merchants:'商家入驻',products:'商品中心',orders:'电瓶车订单',phones:'电话卡订单',recharges:'话费权益',broadband:'宽带资格',plates:'牌照辅助',afterSales:'售后工单',logs:'操作日志',settings:'运营设置'};
...
function merchants(){const q=state.query.toLowerCase();const items=(state.data.merchants||[]).filter(x=>!q||`${x.name} ${x.ownerName} ${x.phone}`.toLowerCase().includes(q));const rows=items.map(x=>`<tr><td><strong>${esc(x.name)}</strong><small>${fmtDate(x.createdAt)}</small></td><td>${esc(x.ownerName)}<small>${esc(x.phone)}</small></td><td>${esc(merchantTypeLabels[x.merchantType]||'个体工商户')}</td><td>${esc(merchantCategoryLabels[x.category]||x.category)}<small>${esc(x.serviceArea||'')}</small></td><td><span class="badge ${badgeClass(x.status)}">${label(x.status)}</span>${x.licenseUrl?`<a class="license-link" href="${esc(x.licenseUrl)}" target="_blank">查看执照</a>`:''}</td><td><div class="row-actions"><button class="text-button detail-button" data-view="merchants" data-id="${x.id}">审核详情</button><button class="text-button" data-merchant="${x.id}" data-status="APPROVED" ${x.status==='APPROVED'?'disabled':''}>通过</button><button class="text-button danger" data-merchant="${x.id}" data-status="REJECTED" ${x.status==='REJECTED'?'disabled':''}>驳回</button></div></td></tr>`);return toolbar(items.length)+table(['商家','联系人','主体类型','类目/区域','资质','操作'],rows,items.length)}
function render(){document.querySelector('#pageTitle').textContent=titles[state.view];document.querySelector('#breadcrumb').textContent=titles[state.view];const views={dashboard,merchants,leads,products,orders,phones,recharges,broadband,plates,afterSales,logs,settings};document.querySelector('#content').innerHTML=views[state.view]();bindView()}
function bindView(){document.querySelector('#listSearch')?.addEventListener('input',e=>{state.query=e.target.value;render();document.querySelector('#listSearch')?.focus()});document.querySelector('#statusFilter')?.addEventListener('change',e=>{state.status=e.target.value;render()});document.querySelector('#addProduct')?.addEventListener('click',()=>openProduct());document.querySelectorAll('.edit-product').forEach(b=>b.addEventListener('click',()=>openProduct(state.data.products.find(p=>p.id===b.dataset.id))));document.querySelectorAll('.save-status').forEach(b=>b.addEventListener('click',()=>saveStatus(b)));document.querySelectorAll('[data-merchant]').forEach(b=>b.addEventListener('click',async()=>{const status=b.dataset.status;const note=status==='REJECTED'?prompt('请填写驳回原因')||'资质材料不符合要求':prompt('请填写审核备注','资质信息已核对')||'';await api(`/api/admin/merchants/${b.dataset.merchant}/status`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({status,reviewNote:note})});showToast('商家状态已更新');await load()}));document.querySelectorAll('.detail-button').forEach(b=>b.addEventListener('click',()=>openDetail(b.dataset.view,b.dataset.id)));document.querySelector('#exportButton')?.addEventListener('click',exportCurrent);document.querySelector('#settingsForm')?.addEventListener('submit',saveSettings);document.querySelectorAll('[data-goto]').forEach(x=>x.addEventListener('click',()=>goView(x.dataset.goto)))}
...
bindView=function(){originalBindView();bindCollab()};
